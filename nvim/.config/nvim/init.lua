...
-- Remap switching to adjacent buffer
...
end, { desc = '[G]it [F]iles (fallback to find)' })
...
-- Harpoon keymaps (duplicate removed, see below)
...
-- Treesitter config for sticky header
-- Diagnostic keymaps
...
-- Configure each server using Neovim's native LSP config API (Neovim 0.11+)
...
vim.keymap.set("n", "<C-9>", function() ui.nav_next() end)
vim.keymap.set("n", "<C-0>", function() ui.nav_prev() end)

-- Neoformat
vim.keymap.set("n", "<leader>neo", "<cmd>Neoformat<cr>")
